...
/// Accounts needed to remove liquidity
#[derive(Accounts)]
pub struct Withdraw<'info> {
    #[account(mut)]
    pub user: Signer<'info>,           // Person removing liquidity
    ...
    pub mint_lp: Account<'info, Mint>,         // LP token mint
    ...
    pub user_lp: Account<'info, TokenAccount>, // User's LP tokens
...
impl<'info> Withdraw<'info> {
    ...
    pub fn withdraw(
        ...
        min_x: u64,  // Min X tokens to get back
        min_y: u64,  // Min Y tokens to get back
    ) -> Result<()> {
        // Safety checks
        ...
        // Send tokens back to user and burn LP tokens
    ...
    /// Send tokens from pool back to user
